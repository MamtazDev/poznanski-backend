...
router.get("/admin", async (req, res) => {
  try {
    ...
    let allProducts = [];
    if (filter) {
      products = await Logo.find({ name: { $regex: filter, $options: "i" } })
        .skip(start)
        .limit(rowsPerPage);
    ...
    return res
      .status(200)
      .json({ logos: products, all: allProducts.length, success: true });
  } catch (err) {
    console.log(err);
    return res.status(400).json({ success: false, error: err });
  }
  ...
  try {
    ...
    if (newData) {
      ...
      if (data?.length) {
        return res
          .status(200)
          .json({ success: false, message: "This Logo is already posted" });
      } else {
        const date = new Date();
        const newLogo = Logo({
          name: newData.name,
        ...
        if (newData.img1 !== `${process.env.publicUrl}${data.img1}`) {
          if (newData.img1 === "") {
            return res.status(200).json({
              success: false,
              message: "Please Upload the light mode logo image.",
            });
          } else {
            const fileType = newData.img1.substring(
              newData.img1.indexOf("/") + 1,
              newData.img1.indexOf(";")
            );
        ...
        if (newData.img2 !== `${process.env.publicUrl}${data.img2}`) {
          if (newData.img2 === "") {
            return res.status(200).json({
              success: false,
              message: "Please Upload the darl mode logo image.",
            });
          } else {
            const fileType = newData.img2.substring(
              newData.img2.indexOf("/") + 1,
              newData.img2.indexOf(";")
            );
      ...
      if (data) {
        ...
        if (newData.img2 !== `${process.env.publicUrl}${data.img2}`) {
          if (newData.img2 === "") {
            return res.status(200).json({
              success: false,
              message: "Please Upload the 2-Logo image.",
            });
          } else {
            const fileType = newData.img2.substring(
              newData.img2.indexOf("/") + 1,
